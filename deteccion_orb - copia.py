...
for i in range(48):
    img = cv2.imread("train/frontal_" + str(i + 1) + ".jpg", 0)
    ...
    for row_number in range(len(kp)):
        ...
        train_kp_angles.append(get_angle_between_keypoint_and_center(kp_x, kp_y, train_images_center_x, train_images_center_y))
...
img_test = cv2.imread("test/test31.jpg", 0)
img_test_painted = cv2.cvtColor(img_test, cv2.COLOR_GRAY2RGB)
...
test_mods = np.array(test_mods, dtype=np.uint8)
test_kp_angles = np.array(test_kp_angles)
...
for r in results:
    for m in r:
        ...
        add_vote_to_acc_matrix(x, y, train_sizes[m.trainIdx], test_sizes[m.queryIdx], train_mods[m.trainIdx],
                               train_kp_angles[m.trainIdx], train_orientations[m.trainIdx], test_orientations[m.queryIdx],
...
for row_number in range(len(matrix_accumulation_evidence)):
    for col_number in range(len(matrix_accumulation_evidence[row_number])):
        if matrix_accumulation_evidence[row_number][col_number] > maxNumber:
            maxNumber = matrix_accumulation_evidence[row_number][col_number]
            best_position = (row_number, col_number)

cv2.circle(img_test_painted, best_position, 20, (0, 255, 0), -1)
cv2.imshow("Car", img_test_painted)
cv2.waitKey()
